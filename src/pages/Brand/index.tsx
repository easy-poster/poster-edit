...
const Brand = () => {
  ...
  const ColorPicker = ({ index, activeColor, handleColorChange }: any) => {
      ...
      <div className="brand-banner">
        ...
        <ul className="banner-list">
          <li className="banner-li">
            ...
            />
            <span className="li-content">上传您独特的品牌logo、字体和颜色</span>
          </li>
          <li className="banner-li">
            <IconFont
              type="icon-duigou"
              style={{ fontSize: '26px', color: '#5fd5a0' }}
            />
            <span className="li-content">更简单的方式保持品牌一致性</span>
          </li>
          <li className="banner-li">
            <IconFont
              type="icon-duigou"
        ...
        <div className="banner-btn">
          ...
          <IconFont
            ...
            style={{ fontSize: '30px', marginLeft: 10, color: '#FFF' }}
          />
        </div>
      </div>
            ...
            <Upload {...uploadImgProps}>
              <div className="logo-item upload-wrap">
                <IconFont
                  ...
                  style={{ fontSize: '56px', color: '#BFBFBF' }}
                />
              </div>
            </Upload>
            {logoList.map((item, index) => {
              return (
                <div className="logo-item" key={logoGetKey(index)}>
                  <img src={item.logo} alt="" />
                  <IconFont
                    className="del"
                    onClick={(event) => handleLogoDel(item, index, event)}
                    type="icon-chacha"
                ...
                >
                  {index === colorList.length - 1 ? (
                    ...
                    >
                      <IconFont
                        type="icon-tianjia_huaban"
                        style={{ fontSize: '42px', color: '#BFBFBF' }}
                      />
                    </div>
                  ) : (
                    <div
                      className="color-item"
                      onClick={() => handleInitColor(item)}
                    >
                      <div
                        className="color-bg"
                      ...
                      <IconFont
                        className="del"
                        onClick={(event) => handleColorDel(item, index, event)}
                        type="icon-chacha"
        ...
        <div className="item-content">
          <div className="font-list">
            ...
            {fontList.map((item, index) => {
              return (
                <div className="font-item" key={fontGetKey(index)}>
                  <div className="font-bg">
                    <div style={{ fontFamily: item.family }}>这是示例文字</div>
                    <div className="tip">{item.title}</div>
                  </div>
                  <IconFont
                    className="del"
                    onClick={(event) => handleFontDel(item, index, event)}
                    type="icon-chacha"
                    style={{ fontSize: '16px', color: '#000' }}
                  />
                </div>
