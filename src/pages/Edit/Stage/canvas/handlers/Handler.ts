import { ActiveSelection, Canvas, Gradient, Object, Pattern } from 'fabric/fabric-impl';
...
class Handler implements HandlerOptions {
    ...
    activeSelectOptions?: Partial<FabricObjectOption<ActiveSelection>> | undefined;
    handlers?: { [key: string]: CustomHandler } | undefined;
    onDbClick?: ((canvas: FabricCanvas<Canvas>, target: FabricObject<Object>) => void) | undefined;
    ...
    public initControls = () => {
        ...
        this.canvas.on('text:changed', () => {
            ...
            if (!isNaN(linewidth) && linewidth + 40 > this.canvas.getActiveObject().width) {
    ...
    public getObjects = (): FabricObject[] => {
        ...
        if (objects.length) {
            objects.forEach((obj: FabricObject) => obj.id && (this.objectMap[obj.id] = obj));
        } else {
            this.objectMap = {};
        }
        return objects;
    ...
    public setImage = (obj: FabricImage, source?: File | string): Promise<FabricImage> => {
        return new Promise((resolve) => {
            if (!source) {
                ...
                obj.set('src', null);
                resolve(
                    obj.setSrc('http://localhost:8000/demo.png', () => this.canvas.renderAll(), {
                        dirty: true,
                    }) as FabricImage,
                );
            }
